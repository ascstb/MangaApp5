...
class ServerRepositoryImpl(
    ...
    override fun getLatestReleasesAsync(page: Int): Deferred<RepositoryResponse<List<Manga>>> =
        when (Session.selectedProvider) {
            ...
            else -> mangaTownProvider.getLatestReleasesAsync(page)
        }
    ...
    override fun getMangaDetailsAsync(path: String): Deferred<RepositoryResponse<Manga>> =
        when (Session.selectedProvider) {
            ...
            else -> mangaTownProvider.getMangaDetailsAsync(path)
        }
    ...
    override fun getMangaPageAsync(
        ...
        else -> mangaTownProvider.getMangaPageAsync(path, pageNumber)
    }

    override fun getMangaSearchResultsAsync(searchParams: SearchParameters): Deferred<RepositoryResponse<List<Manga>>> =
        when (Session.selectedProvider) {
            MangaProvidersEnum.MANGATOWN -> mangaTownProvider.getMangaSearchResultsAsync(
                searchParams
            )
            else -> mangaTownProvider.getMangaSearchResultsAsync(searchParams)
        }
}